...
const CharacterOverrideNameSchema = z.string()
  ...
  .min(1)
  .max(24)
  .refine((value) => !value.includes("@"), "character override names must not include control markers")
...
export const CharacterOverridesSchema = z.object({
  schema: z.literal("swagbound.character-overrides.v1"),
...
const EnemyOverrideNameSchema = z.string()
  .trim()
  .min(1)
  .max(24)
  .refine((value) => !value.includes("@"), "enemy override names must not include control markers")
  .refine((value) => !/[\u0000-\u001f\u007f]/.test(value), "enemy override names must not include control codes");

const EnemyOverrideEntrySchema = z.object({
  name: EnemyOverrideNameSchema
}).strict();
...
export const SwagboundDialogueLibrarySchema = z.object({
  schema: z.literal("swagbound.dialogue-library.v1"),
