...
export const INTRO_TUTORIAL_ENEMY_ID = 900001;
...
export function resolveIntroMeteorBeatStart(
  ...
  // Keep the option name for fixture compatibility; production resolves the
  // dedicated Cold Signal encounter from the opening-clarity overlay.
  const tutorialEnemyId = options.starmanJuniorEnemyId ?? INTRO_TUTORIAL_ENEMY_ID;
  ...
  const battleGroup = battle.groups.find((group) => group.enemyIds.includes(tutorialEnemyId));
  if (!battleGroup) {
