import { afterAction, createFleetRunControl, directionToward, dist, hold, limitFor, pointValid, state, warpTo } from "./shared.mjs";
...
export async function run(ctx) {
  ...
  const watch = createFleetRunControl(ctx, "doors", { total: doors.length, doneLabel: "doors" });
  const session = await ctx.pagePool.acquire("doors");
  try {
    for (const door of doors) {
      const result = await watch.runItem(`door ${door.worldPixel.x},${door.worldPixel.y}`, async () => {
        ...
        await afterAction(ctx, session, session.lastAction);
      }, { at: door.worldPixel, evidence: { door } });
